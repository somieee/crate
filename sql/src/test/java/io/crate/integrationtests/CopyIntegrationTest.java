...
public class CopyIntegrationTest extends SQLHttpIntegrationTest {
    ...
    @Test
    public void testCopyFromFileWithCSVExtension() throws Exception {
        execute("create table quotes (id int primary key, " +
            "quote string index using fulltext) with (number_of_replicas = 0)");
        ensureYellow();

        execute("copy quotes from ?", new Object[]{copyFilePath + "test_copy_from.csv"});
        assertEquals(3L, response.rowCount());
        refresh();
    ...
    @Test
    public void testCopyFromFileWithoutOptionOrRelevantFileExtension() throws Exception {
        execute("create table quotes (id int primary key, " +
            "quote string index using fulltext) with (number_of_replicas = 0)");
        ensureYellow();

        execute("copy quotes from ?", new Object[]{copyFilePath + "test_copy_from_json.ext"});
        assertEquals(3L, response.rowCount());
        refresh();
